...
describe('Branding', () => {
  it('BrandLogo full 变体渲染标准中英文字标并适配暗黑模式', () => {
    ...
    expect(logo).toHaveAttribute('aria-label', '灵境实训 / NEXUS LEARN 品牌标识');
    expect(logo).toHaveClass('text-slate-900');
    expect(logo).toHaveClass('dark:text-white');
    expect(screen.getByText('BA')).toBeInTheDocument();
    expect(screen.getByText('灵境实训')).toBeInTheDocument();
    expect(screen.getByText('NEXUS LEARN')).toBeInTheDocument();
  });
  ...
  it('BrandLogo icon 变体只渲染徽标图形并保留科技蓝暗色状态', () => {
    ...
    expect(screen.getByText('BA')).toBeInTheDocument();
    expect(screen.queryByText('灵境实训')).not.toBeInTheDocument();
  });
  ...
  it('BrandWatermark 渲染可复用防伪水印文案', () => {
    render(<BrandWatermark text="NEXUS 内部资产" data-testid="brand-watermark" />);

    expect(screen.getByTestId('brand-watermark')).toHaveClass('pointer-events-none');
    expect(screen.getAllByText('NEXUS 内部资产').length).toBeGreaterThan(1);
  });
});
